function [stiff diinsy cols sysmat] = sb_calc_stiff(cond, vol)
...
disp("Function sb_calc_stiff changed at lines 51, 63 and 64")  % ADDED
if isfield(vol,'cond') && isfield(vol,'tissue') && isfield(vol,'tissuelabel')
    ...
    elseif length(vol.tissuelabel) < length(vol.cond)  % ADDED
        cond = double(vol.cond);  % ADDED
    else
